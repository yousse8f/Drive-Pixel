'use client';

import Link from 'next/link';
import { ArrowRight, Mail, Database, Users, TrendingUp, Settings, Target } from 'lucide-react';
import { Button } from '@/components/ui/button';
import Navbar from '@/components/Navbar';
import Footer from '@/components/Footer';
import DynamicPageContent from '@/components/DynamicPageContent';

export default function NWMLSAccessPage() {
  const benefits = [
    {
      icon: Database,
      title: 'Extensive Property Database',
      description: 'Access to comprehensive property listings with detailed information and market data.'
    },
    {
      icon: TrendingUp,
      title: 'Market Insights',
      description: 'Real-time market trends, pricing analysis, and neighborhood statistics.'
    },
    {
      icon: Users,
      title: 'Networking Opportunities',
      description: 'Connect with other real estate professionals and expand your industry network.'
    },
    {
      icon: Settings,
      title: 'Professional Tools',
      description: 'Advanced tools for property analysis, client management, and transaction coordination.'
    }
  ];

  const processSteps = [
    {
      title: 'Membership Access',
      description: 'Gain access through One Drive Realty membership program.'
    },
    {
      title: 'Qualification Required',
      description: 'Complete qualification process and receive approval.'
    },
    {
      title: 'Fee & Compliance',
      description: 'Pay required fees and maintain compliance with NWMLS regulations.'
    }
  ];

  return (
    <div className="min-h-screen flex flex-col">
      <Navbar />
      
      {/* Hero Section - CMS Controlled */}
      <DynamicPageContent pagePath="/real-estate/why-onedrive/nwmls-access" fallbackContent={
        <section className="relative py-32 text-white">
          <div 
            className="absolute inset-0 bg-cover bg-center z-0"
            style={{
              backgroundImage: 'url(/images/Real%20Estate.png)',
              backgroundSize: 'cover',
              backgroundPosition: 'center',
              backgroundRepeat: 'no-repeat',
            }}
          >
            <div className="absolute inset-0 bg-[#1a1f3a]/70"></div>
          </div>
          <div className="container-custom text-center relative z-10">
            <h1 className="text-5xl md:text-6xl font-bold mb-6">NWMLS ACCESS</h1>
            <p className="text-lg md:text-xl text-white/90 max-w-3xl mx-auto">
              Access comprehensive property database and professional tools
            </p>
          </div>
        </section>
      } />

      <section className="py-16 bg-off-white">
        <div className="max-w-6xl mx-auto px-4">
          <div className="space-y-6 mb-12">
            <p className="text-lg text-[#4f2c33] leading-relaxed text-center max-w-4xl mx-auto">
              One Drive Realty provides access to NWMLS, even for unlicensed individuals. This unique opportunity allows you to tap into the extensive Northwest Multiple Listing Service database and professional real estate resources.
            </p>
          </div>

          <div className="space-y-4 mb-12">
            <div className="w-full rounded-none bg-pale-goldenrod border border-metallic-gold shadow-[inset_0_4px_0_rgba(255,255,255,0.55)] py-2 text-center uppercase text-[11px] font-semibold tracking-[0.35em] text-midnight-blue">
              HOW IT WORKS
            </div>
            <div className="w-full rounded-none bg-royal-purple border border-muted-indigo shadow-[0_6px_0_rgba(88,70,150,0.35)] py-3 text-center">
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white tracking-tight">
                Access Process
              </h2>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
            {processSteps.map((step, index) => (
              <div key={step.title} className="bg-light-beige border border-muted-indigo/30 rounded-lg p-6 shadow-[0_12px_0_0_rgba(45,58,107,0.3)] text-center relative">
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-sea-green text-white rounded-full w-8 h-8 flex items-center justify-center font-bold shadow-lg">
                  {index + 1}
                </div>
                <h3 className="text-xl font-bold text-midnight-blue mb-4 mt-2">{step.title}</h3>
                <p className="text-muted-indigo leading-relaxed">{step.description}</p>
              </div>
            ))}
          </div>

          <div className="space-y-4 mb-12">
            <div className="w-full rounded-none bg-pale-goldenrod border border-metallic-gold shadow-[inset_0_4px_0_rgba(255,255,255,0.55)] py-2 text-center uppercase text-[11px] font-semibold tracking-[0.35em] text-midnight-blue">
              NWMLS BENEFITS
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
            {benefits.map((benefit) => {
              const Icon = benefit.icon;
              return (
                <div key={benefit.title} className="flex flex-col items-center gap-4 rounded-lg border border-muted-indigo/30 bg-light-beige px-6 py-8 text-center shadow-[0_12px_0_0_rgba(45,58,107,0.3)] hover:-translate-y-2 transition-transform min-h-[280px]">
                  <div className="h-20 w-20 rounded-full bg-sea-green shadow-[inset_0_-6px_0_rgba(0,0,0,0.18)] flex items-center justify-center text-white flex-shrink-0">
                    <Icon className="h-10 w-10" />
                  </div>
                  <h3 className="text-xl font-bold text-midnight-blue leading-tight">{benefit.title}</h3>
                  <p className="text-sm text-muted-indigo leading-relaxed break-words flex-1">{benefit.description}</p>
                </div>
              );
            })}
          </div>

          <div className="space-y-4 mb-12">
            <div className="w-full rounded-none bg-pale-goldenrod border border-metallic-gold shadow-[inset_0_4px_0_rgba(255,255,255,0.55)] py-2 text-center uppercase text-[11px] font-semibold tracking-[0.35em] text-midnight-blue">
              UNIQUE OPPORTUNITY
            </div>
          </div>

          <div className="space-y-6 mb-12">
            <div className="bg-light-beige border border-muted-indigo/30 rounded-lg p-8 shadow-[0_12px_0_0_rgba(45,58,107,0.3)]">
              <div className="text-center space-y-4">
                <div className="h-20 w-20 rounded-full bg-sea-green shadow-[inset_0_-6px_0_rgba(0,0,0,0.18)] flex items-center justify-center text-white mx-auto">
                  <Target className="h-10 w-10" />
                </div>
                <h3 className="text-2xl font-bold text-midnight-blue">Access Without Licensing</h3>
                <p className="text-lg text-muted-indigo leading-relaxed max-w-3xl mx-auto">
                  One Drive Realty's unique partnership with NWMLS allows unlicensed individuals to access the powerful MLS database. This opens up opportunities for real estate enthusiasts, investors, and aspiring agents to explore the market with professional tools.
                </p>
                <p className="text-lg text-muted-indigo leading-relaxed max-w-3xl mx-auto">
                  Whether you're researching properties, monitoring market trends, or preparing for a real estate career, NWMLS access provides invaluable insights and data.
                </p>
              </div>
            </div>
          </div>

          <div className="space-y-4 mb-12">
            <div className="w-full rounded-none bg-pale-goldenrod border border-metallic-gold shadow-[inset_0_4px_0_rgba(255,255,255,0.55)] py-2 text-center uppercase text-[11px] font-semibold tracking-[0.35em] text-midnight-blue">
              REQUIREMENTS
            </div>
          </div>

          <div className="space-y-6 mb-12">
            <div className="bg-light-beige border border-muted-indigo/30 rounded-lg p-6 shadow-[0_12px_0_0_rgba(45,58,107,0.3)]">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-bold text-midnight-blue mb-2">Eligibility Criteria</h4>
                  <ul className="space-y-1 text-muted-indigo">
                    <li className="flex items-start">
                      <span className="text-sea-green mr-2">•</span>
                      One Drive Realty membership
                    </li>
                    <li className="flex items-start">
                      <span className="text-sea-green mr-2">•</span>
                      Background check clearance
                    </li>
                    <li className="flex items-start">
                      <span className="text-sea-green mr-2">•</span>
                      Compliance with NWMLS rules
                    </li>
                  </ul>
                </div>
                <div>
                  <h4 className="font-bold text-midnight-blue mb-2">Ongoing Requirements</h4>
                  <ul className="space-y-1 text-muted-indigo">
                    <li className="flex items-start">
                      <span className="text-sea-green mr-2">•</span>
                      Annual membership fees
                    </li>
                    <li className="flex items-start">
                      <span className="text-sea-green mr-2">•</span>
                      Regular compliance training
                    </li>
                    <li className="flex items-start">
                      <span className="text-sea-green mr-2">•</span>
                      Active membership status
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      <section className="py-16 bg-midnight-blue text-white">
        <div className="max-w-4xl mx-auto px-4 text-center space-y-6">
          <div className="w-full rounded-none bg-pale-goldenrod border border-metallic-gold shadow-[inset_0_4px_0_rgba(255,255,255,0.4)] py-2 uppercase text-[11px] font-extrabold tracking-[0.35em] text-midnight-blue bg-opacity-90">
            GET NWMLS ACCESS
          </div>
          <h2 className="text-[clamp(2rem,4vw,3rem)] font-black">Unlock Professional Real Estate Tools</h2>
          
          <div className="flex flex-col sm:flex-row justify-center gap-4 pt-4">
            <a href="mailto:info@onedriverealty.com">
              <Button size="lg" className="w-full sm:w-auto bg-metallic-gold hover:bg-amber text-midnight-blue font-bold px-8 py-6 text-lg rounded-md transition-all duration-300 transform hover:scale-105 shadow-xl">
                <Mail className="mr-2 h-5 w-5" />
                Email Us
              </Button>
            </a>
            <Link href="/contact">
              <Button size="lg" className="w-full sm:w-auto bg-metallic-gold hover:bg-amber text-midnight-blue font-bold px-8 py-6 text-lg rounded-md transition-all duration-300 transform hover:scale-105 shadow-xl">
                Get Details
                <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
            </Link>
          </div>
        </div>
      </section>

      <Footer />
    </div>
  );
}
