...
exports.getEmailStats = exports.unsubscribe = exports.trackEmailClick = exports.trackEmailOpen = exports.sendTestEmail = exports.sendCampaign = exports.deleteEmailCampaign = exports.updateEmailCampaign = exports.createEmailCampaign = exports.getEmailCampaign = exports.getEmailCampaigns = exports.deleteEmailTemplate = exports.updateEmailTemplate = exports.createEmailTemplate = exports.getEmailTemplate = exports.getEmailTemplates = exports.bulkAddSubscribers = exports.removeSubscriber = exports.addSubscriber = exports.getSubscribers = exports.deleteEmailList = exports.updateEmailList = exports.createEmailList = exports.getEmailList = exports.getEmailLists = void 0;
...
const getEmailLists = async (req, res) => {
    try {
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM email_lists l ${whereClause}`, params);
        params.push(Number(limit), offset);
        const result = await (0, database_1.query)(`SELECT l.*, 
...
const getEmailList = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, database_1.query)(`SELECT l.*, u.email as created_by_email
       FROM email_lists l
       LEFT JOIN users u ON l.created_by = u.id
       WHERE l.id = $1`, [id]);
        ...
        const statsResult = await (0, database_1.query)(`SELECT status, COUNT(*) as count
...
const createEmailList = async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`INSERT INTO email_lists (name, description, type, segment_criteria, created_by)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [name, description, type || 'manual', segment_criteria || {}, userId]);
        await (0, auditLogger_1.createAuditLog)(userId, "create", "email_list", result.rows[0].id, null, result.rows[0], req);
...
const updateEmailList = async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const oldResult = await (0, database_1.query)("SELECT * FROM email_lists WHERE id = $1", [id]);
        if (oldResult.rows.length === 0) {
            return res.status(404).json({ success: false, message: "Email list not found" });
        }
        const result = await (0, database_1.query)(`UPDATE email_lists 
       ...
       WHERE id = $6
       RETURNING *`, [name, description, type, segment_criteria, is_active, id]);
        await (0, auditLogger_1.createAuditLog)(userId, "update", "email_list", id, oldResult.rows[0], result.rows[0], req);
...
exports.updateEmailList = updateEmailList;
const deleteEmailList = async (req, res) => {
    try {
        ...
        await (0, database_1.query)("DELETE FROM email_lists WHERE id = $1", [id]);
        await (0, auditLogger_1.createAuditLog)(userId, "delete", "email_list", id, oldResult.rows[0], null, req);
...
const getSubscribers = async (req, res) => {
    try {
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM email_list_subscribers s ${whereClause}`, params);
        params.push(Number(limit), offset);
        const result = await (0, database_1.query)(`SELECT s.*, l.name as list_name
...
const addSubscriber = async (req, res) => {
    try {
        ...
        const existing = await (0, database_1.query)("SELECT id, status FROM email_list_subscribers WHERE list_id = $1 AND email = $2", [list_id, email]);
        if (existing.rows.length > 0) {
            if (existing.rows[0].status === 'active') {
                return res.status(400).json({ success: false, message: "Email already subscribed to this list" });
            }
            // Reactivate
            const result = await (0, database_1.query)(`UPDATE email_list_subscribers 
        ...
        const result = await (0, database_1.query)(`INSERT INTO email_list_subscribers (list_id, email, first_name, last_name, customer_id, custom_fields)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [list_id, email, first_name, last_name, customer_id, custom_fields || {}]);
...
const bulkAddSubscribers = async (req, res) => {
    try {
        ...
        for (const sub of subscribers) {
            try {
                ...
                const existing = await (0, database_1.query)("SELECT id FROM email_list_subscribers WHERE list_id = $1 AND email = $2", [list_id, sub.email]);
                if (existing.rows.length > 0) {
                    skipped++;
                    continue;
                }
                await (0, database_1.query)(`INSERT INTO email_list_subscribers (list_id, email, first_name, last_name, custom_fields)
           VALUES ($1, $2, $3, $4, $5)`, [list_id, sub.email, sub.first_name, sub.last_name, sub.custom_fields || {}]);
                added++;
            }
            catch (err) {
...
exports.bulkAddSubscribers = bulkAddSubscribers;
// =====================================================
// EMAIL TEMPLATES
// =====================================================
const getEmailTemplates = async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT t.*, u.email as created_by_email
...
const createEmailTemplate = async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`INSERT INTO email_templates (name, subject, html_content, text_content, variables, category, created_by, updated_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $7)
       RETURNING *`, [name, subject, html_content, text_content, variables || [], category, userId]);
        await (0, auditLogger_1.createAuditLog)(userId, "create", "email_template", result.rows[0].id, null, result.rows[0], req);
        res.status(201).json({ success: true, data: result.rows[0] });
    }
    catch (error) {
...
const updateEmailTemplate = async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const oldResult = await (0, database_1.query)("SELECT * FROM email_templates WHERE id = $1", [id]);
        if (oldResult.rows.length === 0) {
            return res.status(404).json({ success: false, message: "Template not found" });
        }
        const result = await (0, database_1.query)(`UPDATE email_templates 
       SET name = COALESCE($1, name),
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE id = $9
       RETURNING *`, [name, subject, html_content, text_content, variables, category, is_active, userId, id]);
        await (0, auditLogger_1.createAuditLog)(userId, "update", "email_template", id, oldResult.rows[0], result.rows[0], req);
        res.json({ success: true, data: result.rows[0] });
    }
    catch (error) {
...
const deleteEmailTemplate = async (req, res) => {
    try {
        ...
        await (0, database_1.query)("DELETE FROM email_templates WHERE id = $1", [id]);
        await (0, auditLogger_1.createAuditLog)(userId, "delete", "email_template", id, oldResult.rows[0], null, req);
...
const getEmailCampaigns = async (req, res) => {
    try {
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM email_campaigns c ${whereClause}`, params);
        params.push(Number(limit), offset);
...
const getEmailCampaign = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, database_1.query)(`SELECT c.*, 
        l.name as list_name,
...
const createEmailCampaign = async (req, res) => {
    try {
        ...
        const subscriberCount = await (0, database_1.query)("SELECT COUNT(*) FROM email_list_subscribers WHERE list_id = $1 AND status = 'active'", [list_id]);
        const result = await (0, database_1.query)(`INSERT INTO email_campaigns (name, subject, template_id, list_id, html_content, text_content, scheduled_at, total_recipients, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING *`, [name, subject, template_id, list_id, html_content, text_content, scheduled_at, parseInt(subscriberCount.rows[0].count), userId]);
        await (0, auditLogger_1.createAuditLog)(userId, "create", "email_campaign", result.rows[0].id, null, result.rows[0], req);
        res.status(201).json({ success: true, data: result.rows[0] });
    }
    catch (error) {
...
const updateEmailCampaign = async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const oldResult = await (0, database_1.query)("SELECT * FROM email_campaigns WHERE id = $1", [id]);
        if (oldResult.rows.length === 0) {
            return res.status(404).json({ success: false, message: "Campaign not found" });
        }
        ...
        const result = await (0, database_1.query)(`UPDATE email_campaigns 
       SET name = COALESCE($1, name),
           subject = COALESCE($2, subject),
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE id = $9
       RETURNING *`, [name, subject, template_id, list_id, html_content, text_content, scheduled_at, status, id]);
        await (0, auditLogger_1.createAuditLog)(userId, "update", "email_campaign", id, oldResult.rows[0], result.rows[0], req);
        res.json({ success: true, data: result.rows[0] });
    }
    catch (error) {
...
const deleteEmailCampaign = async (req, res) => {
    try {
        ...
        await (0, database_1.query)("DELETE FROM email_campaigns WHERE id = $1", [id]);
        await (0, auditLogger_1.createAuditLog)(userId, "delete", "email_campaign", id, oldResult.rows[0], null, req);
...
const sendCampaign = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.userId;
        const campaignResult = await (0, database_1.query)("SELECT * FROM email_campaigns WHERE id = $1", [id]);
        ...
        await (0, database_1.query)("UPDATE email_campaigns SET status = 'sending' WHERE id = $1", [id]);
        // Get subscribers
        const subscribersResult = await (0, database_1.query)(`SELECT * FROM email_list_subscribers 
       WHERE list_id = $1 AND status = 'active'`, [campaign.list_id]);
        // Create send log entries
        for (const subscriber of subscribersResult.rows) {
            await (0, database_1.query)(`INSERT INTO email_send_log (campaign_id, subscriber_id, email, status)
         VALUES ($1, $2, $3, 'pending')`, [id, subscriber.id, subscriber.email]);
        }
        // Start sending in background
        sendEmailsInBackground(id, campaign, subscribersResult.rows);
        await (0, auditLogger_1.createAuditLog)(userId, "send_email", "email_campaign", id, null, { status: 'sending' }, req);
...
const sendTestEmail = async (req, res) => {
    try {
        ...
        const campaignResult = await (0, database_1.query)("SELECT * FROM email_campaigns WHERE id = $1", [campaign_id]);
        if (campaignResult.rows.length === 0) {
            return res.status(404).json({ success: false, message: "Campaign not found" });
        }
        const campaign = campaignResult.rows[0];
...
const trackEmailOpen = async (req, res) => {
    try {
        const { id } = req.params;
        await (0, database_1.query)(`UPDATE email_send_log 
       SET status = 'opened', opened_at = CURRENT_TIMESTAMP
       WHERE id = $1 AND opened_at IS NULL`, [id]);
        // Update campaign open count
        const logResult = await (0, database_1.query)("SELECT campaign_id FROM email_send_log WHERE id = $1", [id]);
        if (logResult.rows.length > 0) {
            await (0, database_1.query)("UPDATE email_campaigns SET open_count = open_count + 1 WHERE id = $1", [logResult.rows[0].campaign_id]);
        }
...
const trackEmailClick = async (req, res) => {
    try {
        ...
        // Log click
        await (0, database_1.query)(`INSERT INTO email_click_tracking (send_log_id, url, ip_address, user_agent)
       VALUES ($1, $2, $3, $4)`, [id, url, req.ip, req.headers['user-agent']]);
        // Update send log
        await (0, database_1.query)(`UPDATE email_send_log 
       ...
       WHERE id = $1`, [id]);
        ...
        const logResult = await (0, database_1.query)("SELECT campaign_id FROM email_send_log WHERE id = $1", [id]);
        if (logResult.rows.length > 0) {
            await (0, database_1.query)("UPDATE email_campaigns SET click_count = click_count + 1 WHERE id = $1", [logResult.rows[0].campaign_id]);
        }
...
const unsubscribe = async (req, res) => {
    try {
        ...
        if (list_id) {
         ...
         SET status = 'unsubscribed', unsubscribed_at = CURRENT_TIMESTAMP
         WHERE email = $1 AND list_id = $2`, [email, list_id]);
        }
        else {
        ...
        // Log unsubscribe
        await (0, database_1.query)(`INSERT INTO email_unsubscribes (email, list_id, campaign_id)
       VALUES ($1, $2, $3)`, [email, list_id || null, campaign_id || null]);
        // Update campaign unsubscribe count
        if (campaign_id) {
            await (0, database_1.query)("UPDATE email_campaigns SET unsubscribe_count = unsubscribe_count + 1 WHERE id = $1", [campaign_id]);
        }
...
const getEmailStats = async (req, res) => {
    ...
    `);
        ...
        const recentCampaigns = await (0, database_1.query)(`
    ...
    `);
        ...
        const subscriberGrowth = await (0, database_1.query)(`
...
// HELPER FUNCTIONS
// =====================================================
async function updateSubscriberCount(listId) {
    try {
        const result = await (0, database_1.query)("SELECT COUNT(*) FROM email_list_subscribers WHERE list_id = $1 AND status = 'active'", [listId]);
        await (0, database_1.query)("UPDATE email_lists SET subscriber_count = $1 WHERE id = $2", [parseInt(result.rows[0].count), listId]);
    }
    catch (error) {
...
async function sendEmailsInBackground(campaignId, campaign, subscribers) {
    const transporter = createTransporter();
    let sentCount = 0;
    let bounceCount = 0;
    ...
    for (const subscriber of subscribers) {
        try {
            ...
            // Add tracking pixel
            const sendLogResult = await (0, database_1.query)("SELECT id FROM email_send_log WHERE campaign_id = $1 AND subscriber_id = $2", [campaignId, subscriber.id]);
            ...
            // Update send log
            await (0, database_1.query)("UPDATE email_send_log SET status = 'sent', sent_at = CURRENT_TIMESTAMP WHERE id = $1", [sendLogId]);
            sentCount++;
            // Rate limiting delay
            ...
            await (0, database_1.query)(`UPDATE email_send_log SET status = 'failed', error_message = $1 WHERE campaign_id = $2 AND subscriber_id = $3`, [error.message, campaignId, subscriber.id]);
            if (error.responseCode === 550 || error.responseCode === 551) {
                bounceCount++;
                // Mark subscriber as bounced
                await (0, database_1.query)("UPDATE email_list_subscribers SET status = 'bounced' WHERE id = $1", [subscriber.id]);
            }
        }
    }
    // Update campaign status
    await (0, database_1.query)(`UPDATE email_campaigns 
     SET status = 'sent', sent_at = CURRENT_TIMESTAMP, sent_count = $1, bounce_count = $2
     WHERE id = $3`, [sentCount, bounceCount, campaignId]);
}
