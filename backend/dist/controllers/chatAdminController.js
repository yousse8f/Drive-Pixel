...
exports.listChatMessages = exports.listChatSessions = void 0;
...
const listChatSessions = async (req, res) => {
    ...
    `;
        ...
        const [sessionsResult, countResult] = await Promise.all([
            (0, database_1.query)(sessionsQuery, params),
            (0, database_1.query)(countQuery, params),
        ]);
        const total = Number(countResult.rows[0]?.count || 0);
        return res.json((0, apiResponse_1.successResponse)("Chat sessions retrieved", {
            sessions: sessionsResult.rows,
            pagination: { page, limit, total },
        }));
    }
    catch (error) {
        return res.status(500).json((0, apiResponse_1.errorResponse)("Failed to load chat sessions", error.message));
    }
};
...
const listChatMessages = async (req, res) => {
    try {
        const { id } = req.params;
        const messagesResult = await (0, database_1.query)(`SELECT id, sender, message, page_url, created_at
       FROM chat_messages
       WHERE session_id = $1
       ORDER BY created_at ASC`, [id]);
        return res.json((0, apiResponse_1.successResponse)("Chat messages retrieved", messagesResult.rows));
    }
    catch (error) {
        return res.status(500).json((0, apiResponse_1.errorResponse)("Failed to load chat messages", error.message));
    }
};
