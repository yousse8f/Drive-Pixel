...
exports.restoreContentVersion = exports.getContentVersions = exports.deleteComponent = exports.updateComponent = exports.createComponent = exports.getComponents = exports.deleteContentBlock = exports.updateContentBlock = exports.createContentBlock = exports.getContentBlocks = exports.deleteSection = exports.updateSection = exports.createSection = exports.getSections = exports.deletePage = exports.updatePage = exports.createPage = exports.getPage = exports.getPages = void 0;
...
const getPages = async (req, res) => {
    try {
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM cms_pages ${whereClause}`, params);
...
const getPage = async (req, res) => {
    try {
       ...
       WHERE p.id = $1`, [id]);
        ...
        const sectionsResult = await (0, database_1.query)(`SELECT s.*, 
...
const createPage = async (req, res) => {
    try {
        ...
        const existing = await (0, database_1.query)("SELECT id FROM cms_pages WHERE slug = $1", [slug]);
        if (existing.rows.length > 0) {
            return res.status(400).json({ success: false, message: "Slug already exists" });
        }
        const result = await (0, database_1.query)(`INSERT INTO cms_pages (slug, title, meta_title, meta_description, status, template, created_by, updated_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $7)
       RETURNING *`, [slug, title, meta_title, meta_description, status || "draft", template || "default", userId]);
        await (0, auditLogger_1.createAuditLog)(userId, "create", "cms_page", result.rows[0].id, null, result.rows[0], req);
...
const updatePage = async (req, res) => {
    try {
        ...
        const oldResult = await (0, database_1.query)("SELECT * FROM cms_pages WHERE id = $1", [id]);
        if (oldResult.rows.length === 0) {
            return res.status(404).json({ success: false, message: "Page not found" });
        }
        // Check if new slug conflicts
        if (slug && slug !== oldResult.rows[0].slug) {
            const existing = await (0, database_1.query)("SELECT id FROM cms_pages WHERE slug = $1 AND id != $2", [slug, id]);
            if (existing.rows.length > 0) {
                return res.status(400).json({ success: false, message: "Slug already exists" });
            }
        }
        ...
        const result = await (0, database_1.query)(`UPDATE cms_pages 
       ...
       WHERE id = $9
       RETURNING *`, [slug, title, meta_title, meta_description, status, template, publishedAt, userId, id]);
        await (0, auditLogger_1.createAuditLog)(userId, "update", "cms_page", id, oldResult.rows[0], result.rows[0], req);
...
const deletePage = async (req, res) => {
    try {
        ...
        await (0, database_1.query)("DELETE FROM cms_pages WHERE id = $1", [id]);
        await (0, auditLogger_1.createAuditLog)(userId, "delete", "cms_page", id, oldResult.rows[0], null, req);
...
const createSection = async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`INSERT INTO cms_sections (page_id, name, section_type, content, "order")
       VALUES ($1, $2, $3, $4, COALESCE($5, 0))
       RETURNING *`, [page_id, name, section_type, JSON.stringify(sanitizedContent), order]);
        await (0, auditLogger_1.createAuditLog)(userId, "create", "cms_section", result.rows[0].id, null, result.rows[0], req);
        res.status(201).json({ success: true, data: result.rows[0] });
    }
    catch (error) {
...
const updateSection = async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const oldResult = await (0, database_1.query)("SELECT * FROM cms_sections WHERE id = $1", [id]);
        if (oldResult.rows.length === 0) {
        ...
        const result = await (0, database_1.query)(`UPDATE cms_sections 
       SET name = COALESCE($1, name),
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE id = $6
       RETURNING *`, [name, section_type, JSON.stringify(sanitizedContent), order, is_active, id]);
        await (0, auditLogger_1.createAuditLog)(userId, "update", "cms_section", id, oldResult.rows[0], result.rows[0], req);
        res.json({ success: true, data: result.rows[0] });
    }
    catch (error) {
...
const deleteSection = async (req, res) => {
    try {
        ...
        await (0, database_1.query)("DELETE FROM cms_sections WHERE id = $1", [id]);
        await (0, auditLogger_1.createAuditLog)(userId, "delete", "cms_section", id, oldResult.rows[0], null, req);
...
const getContentBlocks = async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT * FROM cms_content_blocks ${whereClause} ORDER BY "order"`, params);
        res.json({ success: true, data: result.rows });
    }
    catch (error) {
...
const createContentBlock = async (req, res) => {
    try {
        ...
        const sanitizedContent = content ? sanitizeHtmlContent(content) : {};
        const result = await (0, database_1.query)(`INSERT INTO cms_content_blocks (section_id, block_type, content, "order")
       VALUES ($1, $2, $3, COALESCE($4, 0))
       RETURNING *`, [section_id, block_type, JSON.stringify(sanitizedContent), order]);
        await (0, auditLogger_1.createAuditLog)(userId, "create", "cms_content_block", result.rows[0].id, null, result.rows[0], req);
        res.status(201).json({ success: true, data: result.rows[0] });
    }
    catch (error) {
...
const updateContentBlock = async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const oldResult = await (0, database_1.query)("SELECT * FROM cms_content_blocks WHERE id = $1", [id]);
        if (oldResult.rows.length === 0) {
        ...
        const sanitizedContent = content ? sanitizeHtmlContent(content) : oldResult.rows[0].content;
        const result = await (0, database_1.query)(`UPDATE cms_content_blocks 
       SET block_type = COALESCE($1, block_type),
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE id = $5
       RETURNING *`, [block_type, JSON.stringify(sanitizedContent), order, is_active, id]);
        await (0, auditLogger_1.createAuditLog)(userId, "update", "cms_content_block", id, oldResult.rows[0], result.rows[0], req);
        res.json({ success: true, data: result.rows[0] });
    }
    catch (error) {
...
const deleteContentBlock = async (req, res) => {
    try {
        ...
        await (0, database_1.query)("DELETE FROM cms_content_blocks WHERE id = $1", [id]);
        await (0, auditLogger_1.createAuditLog)(userId, "delete", "cms_content_block", id, oldResult.rows[0], null, req);
...
exports.deleteContentBlock = deleteContentBlock;
// =====================================================
...
const createComponent = async (req, res) => {
    try {
        ...
        const existing = await (0, database_1.query)("SELECT id FROM cms_components WHERE slug = $1", [slug]);
        if (existing.rows.length > 0) {
            return res.status(400).json({ success: false, message: "Slug already exists" });
        }
        const sanitizedContent = content ? sanitizeHtmlContent(content) : {};
        const result = await (0, database_1.query)(`INSERT INTO cms_components (name, slug, component_type, content, created_by, updated_by)
       VALUES ($1, $2, $3, $4, $5, $5)
       RETURNING *`, [name, slug, component_type, JSON.stringify(sanitizedContent), userId]);
        await (0, auditLogger_1.createAuditLog)(userId, "create", "cms_component", result.rows[0].id, null, result.rows[0], req);
        res.status(201).json({ success: true, data: result.rows[0] });
    }
    catch (error) {
...
const updateComponent = async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const oldResult = await (0, database_1.query)("SELECT * FROM cms_components WHERE id = $1", [id]);
        if (oldResult.rows.length === 0) {
            return res.status(404).json({ success: false, message: "Component not found" });
        }
        if (slug && slug !== oldResult.rows[0].slug) {
            const existing = await (0, database_1.query)("SELECT id FROM cms_components WHERE slug = $1 AND id != $2", [slug, id]);
            if (existing.rows.length > 0) {
                return res.status(400).json({ success: false, message: "Slug already exists" });
            }
        }
        ...
        const sanitizedContent = content ? sanitizeHtmlContent(content) : oldResult.rows[0].content;
        const result = await (0, database_1.query)(`UPDATE cms_components 
       SET name = COALESCE($1, name),
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE id = $7
       RETURNING *`, [name, slug, component_type, JSON.stringify(sanitizedContent), is_active, userId, id]);
        await (0, auditLogger_1.createAuditLog)(userId, "update", "cms_component", id, oldResult.rows[0], result.rows[0], req);
        res.json({ success: true, data: result.rows[0] });
    }
    catch (error) {
...
const deleteComponent = async (req, res) => {
    try {
        ...
        await (0, database_1.query)("DELETE FROM cms_components WHERE id = $1", [id]);
        await (0, auditLogger_1.createAuditLog)(userId, "delete", "cms_component", id, oldResult.rows[0], null, req);
...
const getContentVersions = async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`SELECT v.*, u.email as created_by_email
       FROM cms_content_versions v
       LEFT JOIN users u ON v.created_by = u.id
       WHERE v.entity_type = $1 AND v.entity_id = $2
       ORDER BY v.version_number DESC`, [entity_type, entity_id]);
        res.json({ success: true, data: result.rows });
    }
    catch (error) {
...
const restoreContentVersion = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.userId;
        const versionResult = await (0, database_1.query)("SELECT * FROM cms_content_versions WHERE id = $1", [id]);
        ...
        const currentResult = await (0, database_1.query)(`SELECT * FROM ${tableName} WHERE id = $1`, [entity_id]);
        ...
        await (0, database_1.query)(`UPDATE ${tableName} SET ${updateFields}, updated_at = CURRENT_TIMESTAMP WHERE id = $${updateValues.length + 1}`, [...updateValues, entity_id]);
        await (0, auditLogger_1.createAuditLog)(userId, "restore", entity_type, entity_id, currentResult.rows[0], contentObj, req);
...
// HELPER FUNCTIONS
// =====================================================
async function createContentVersion(entityType, entityId, content, userId) {
    try {
        // Get next version number
        const versionResult = await (0, database_1.query)(`SELECT COALESCE(MAX(version_number), 0) + 1 as next_version
       FROM cms_content_versions
       WHERE entity_type = $1 AND entity_id = $2`, [entityType, entityId]);
        const nextVersion = versionResult.rows[0].next_version;
        await (0, database_1.query)(`INSERT INTO cms_content_versions (entity_type, entity_id, version_number, content, created_by)
       VALUES ($1, $2, $3, $4, $5)`, [entityType, entityId, nextVersion, JSON.stringify(content), userId]);
    }
    catch (error) {
...
function sanitizeHtmlContent(content) {
    if (typeof content === "string") {
        return (0, sanitizer_1.sanitizeHtml)(content);
    }
