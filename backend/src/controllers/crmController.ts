...
export const createCustomer = async (req: Request, res: Response) => {
  try {
    const {
      lead_id, first_name, last_name, email, phone, company, job_title,
      address, city, state, country, postal_code, status, source, assigned_to, tags, custom_fields
    } = req.body;
    ...
    const result = await query(
      `INSERT INTO crm_customers 
       ...
       RETURNING *`,
      [lead_id, first_name, last_name, email, phone, company, job_title, address, city, state, country, postal_code, status || 'new', source, assigned_to, tags || [], custom_fields || {}]
    );

    // Create activity
    await createActivity(result.rows[0].id, userId, 'customer_created', 'Customer Created', 'Customer record was created');

    await createAuditLog(userId, "create", "crm_customer", result.rows[0].id, null, result.rows[0], req);
...
export const updateCustomer = async (req: Request, res: Response) => {
  try {
    ...
    const oldResult = await query("SELECT * FROM crm_customers WHERE id = $1", [id]);
    ...
    const result = await query(
      `UPDATE crm_customers 
       ...
       WHERE id = $17
       RETURNING *`,
      [first_name, last_name, email, phone, company, job_title, address, city, state, country, postal_code, status, source, assigned_to, tags, custom_fields, id]
    );
    ...
    await createAuditLog(userId, "update", "crm_customer", id, oldCustomer, result.rows[0], req);
...
export const deleteCustomer = async (req: Request, res: Response) => {
  try {
    ...
    await createAuditLog(userId, "delete", "crm_customer", id, oldResult.rows[0], null, req);
...
export const convertLeadToCustomer = async (req: Request, res: Response) => {
  try {
    ...
    const existingCustomer = await query("SELECT id FROM crm_customers WHERE lead_id = $1", [lead_id]);
    ...
    const result = await query(
      `INSERT INTO crm_customers (lead_id, first_name, email, phone, status, source)
    ...
    // Update lead status
    await query("UPDATE leads SET status = 'converted' WHERE id = $1", [lead_id]);
  ...
  try {
    ...
    const result = await query(
      `INSERT INTO crm_customer_notes (customer_id, user_id, note, is_private)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [customer_id, userId, note, is_private || false]
    );
...
export const deleteNote = async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      "DELETE FROM crm_customer_notes WHERE id = $1 AND user_id = $2 RETURNING customer_id",
      [id, userId]
    );
...
export const logActivity = async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      `INSERT INTO crm_activities (customer_id, user_id, activity_type, title, description, metadata)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [customer_id, userId, activity_type, title, description, metadata || {}]
    );
...
export const createPipelineStage = async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      `INSERT INTO crm_pipeline_stages (name, slug, color, "order")
       VALUES ($1, $2, $3, COALESCE($4, 0))
       RETURNING *`,
      [name, slug, color || '#3B82F6', order]
    );
...
export const getDeals = async (req: Request, res: Response) => {
  try {
    ...
    const countResult = await query(
      `SELECT COUNT(*) FROM crm_deals d ${whereClause}`,
      params
    );
...
export const createDeal = async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      `INSERT INTO crm_deals (customer_id, stage_id, title, value, probability, expected_close_date, assigned_to, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING *`,
      [customer_id, stage_id, title, value || 0, probability || 50, expected_close_date, assigned_to || userId, notes]
    );

    await createActivity(customer_id, userId, 'deal_created', 'Deal Created', `Deal "${title}" was created`);

    await createAuditLog(userId, "create", "crm_deal", result.rows[0].id, null, result.rows[0], req);
...
export const updateDeal = async (req: Request, res: Response) => {
  try {
    ...
    const oldResult = await query("SELECT * FROM crm_deals WHERE id = $1", [id]);
    if (oldResult.rows.length === 0) {
    ...
    const result = await query(
      `UPDATE crm_deals 
       ...
       WHERE id = $9
       RETURNING *`,
      [stage_id, title, value, probability, expected_close_date, assigned_to, status, notes, id]
    );
    ...
    if (status && status !== oldDeal.status) {
      ...
      if (status === 'won') {
        await query(
          `UPDATE crm_customers SET lifetime_value = lifetime_value + $1 WHERE id = $2`,
          [result.rows[0].value, oldDeal.customer_id]
        );
      }
    }

    await createAuditLog(userId, "update", "crm_deal", id, oldDeal, result.rows[0], req);
...
export const deleteDeal = async (req: Request, res: Response) => {
  try {
    ...
    await query("DELETE FROM crm_deals WHERE id = $1", [id]);

    await createAuditLog(userId, "delete", "crm_deal", id, oldResult.rows[0], null, req);
...
export const getCrmStats = async (req: Request, res: Response) => {
  try {
    ...
    // Pipeline overview
    const pipelineOverview = await query(`
...
async function createActivity(customerId: string, userId: string, activityType: string, title: string, description: string, metadata: any = {}) {
  try {
    await query(
      `INSERT INTO crm_activities (customer_id, user_id, activity_type, title, description, metadata)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [customerId, userId, activityType, title, description, metadata]
    );
  } catch (error) {
